...
summary: HTTPまたはHTTPSを使用してAutoSupportパッケージを送信する場合は、管理ノードとテクニカルサポート（AutoSupport）の間に非透過型プロキシサーバを設定できます。 
---
= StorageGRIDで管理プロキシ設定を構成する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
AutoSupportの詳細については、link:../monitor/configure-autosupport-grid-manager.html["AutoSupportの設定"]を参照してください。
