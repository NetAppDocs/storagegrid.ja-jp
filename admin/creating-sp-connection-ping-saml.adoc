---
permalink: admin/creating-sp-connection-ping-saml.html 
sidebar: sidebar 
...
---
= StorageGRID の SAML 用に PingFederate でサービスプロバイダー接続を作成する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
PingFederate を使用して、システム内の SAML 用の各管理ノードのサービスプロバイダー （SP） 接続を作成します。処理を高速化するために、StorageGRID から SAML メタデータをインポートします。

.開始する前に
* StorageGRID にシングルサインオンを設定し、 SSO タイプとして「 Ping federate * 」を選択しました。
* Grid Manager に link:../admin/configure-sso-saml.html#enter-sandbox-mode["サンドボックスモードに入りました"] があります。
* システム内の各管理ノードには * SP接続 ID* があります。これらの値は、「SSO の構成」ページの「管理ノード」の詳細テーブルで確認できます。
...
[[create-password-credential-validatorpassword-validator]]
...
[[create-idp-adapter-instanceadapter-instance]]
...
. 選択してください<<password-validator,パスワードクレデンシャルバリデータ>>あなたが作成したもの。
...
[[create-or-import-signing-certificatesigning-certificate]]
...
. 管理ノードの SSO の構成ページからダウンロードした SAML メタデータ ファイルを選択します。
