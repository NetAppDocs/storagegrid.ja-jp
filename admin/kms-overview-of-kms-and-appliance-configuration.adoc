...
summary: キー管理サーバー（KMS）を使用してアプライアンスノード上の StorageGRID データのセキュリティを確保するには、事前に1つ以上の KMS サーバーを設定し、アプライアンスノードのノード暗号化を有効にする必要があります。これら2つの設定作業が完了すると、鍵管理プロセスが自動的に実行されます。 
---
= StorageGRID キー管理サーバとアプライアンスの設定
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:kms_configuration_overview.png["次のテキストで説明するKMS構成ワークフロー"]
