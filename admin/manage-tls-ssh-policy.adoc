...
+
image:securitysettings_tls_ssh_policies_current.png["TLSおよびSSHポリシー"]
...
.FIPS strict
--
Common Criteria認証が必要で、ロードバランサエンドポイント、テナントマネージャ、およびグリッドマネージャへの外部クライアント接続にNetApp Cryptographic Security Module（NCSM）3.1.2またはNetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1モジュールを使用する必要がある場合は、FIPSの厳格なポリシーを使用してください。このポリシーを使用すると、パフォーマンスが低下する可能性があります。

NCSM 3.1.2 と NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 モジュールは、以下の操作で使用されます。
...
+
詳細については、NIST 暗号化アルゴリズム検証プログラム link:https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/details?validation=40426["証明書番号A7816"^] を参照してください。
...
*** ノード暗号化
+
※注記：このモジュールは認証申請されていません。



+
詳細については、NIST 暗号化アルゴリズム検証プログラム link:https://csrc.nist.gov/projects/cryptographic-module-validation-program/entropy-validations/certificate/223["エントロピー証明書 #E223"^] を参照してください。
...
オプションとして、StorageGRID に FIPS 140 暗号化要件がある場合は、FIPS モード機能を有効にして NCSM 3.1.2 および NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 モジュールを使用してください：
...
+
image:securitysettings_custom_security_policy_copy.png["既存のポリシーをコピーしてカスタムポリシーを作成する"]
