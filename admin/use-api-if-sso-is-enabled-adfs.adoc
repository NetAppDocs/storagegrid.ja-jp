...
[role="lead"]
もしあなたがlink:../admin/how-sso-works.html["シングルサインオン（ SSO ）の設定と有効化"]Active Directory を SSO プロバイダーとして使用する場合は、一連の API 要求を発行して、グリッド管理 API またはテナント管理 API に有効な認証トークンを取得する必要があります。
...
+
image:sso_auth_python_script_adfs.png["AD FS 用 SSO 認証スクリプト"]
