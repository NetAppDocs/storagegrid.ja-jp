...
+
.. StorageGRID システムでシングル サインオン（SSO）が有効になっている場合は、新しい管理ノードの証明書利用者信頼を作成してください。この証明書利用者信頼を作成するまで、ノードにサインインすることはできません。SSO の設定については、link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] または link:../admin/learn-about-setting-up-sso-saml.html["SAML"] を参照してください。
