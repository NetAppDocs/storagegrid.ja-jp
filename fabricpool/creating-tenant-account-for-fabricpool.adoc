...
. [Define root access]手順では、StorageGRIDシステムでが使用されているlink:../admin/using-identity-federation.html["アイデンティティフェデレーション"]か、link:../admin/how-sso-works.html["シングルサインオン（SSO）"]またはその両方に基づいて、テナントアカウントに対する最初のRootアクセス権限を割り当てるユーザを指定します。
+
[cols="1a,2a"]
|===
| オプション | 手順 
