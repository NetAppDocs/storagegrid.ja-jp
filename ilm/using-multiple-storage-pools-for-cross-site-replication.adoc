...
---
= StorageGRID のストレージプールによるサイト損失保護
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:ilm_replication_at_2_sites.png["それぞれ1つのコピーを2つのサイトにレプリケート"]

image:ilm_replication_make_2_copies_2_pools_2_sites.png["サイト固有の2つのストレージプールにそれぞれ1つのコピーを作成します"]
...
image:ilm_erasure_coding_site_loss_protection_4+2.png["EC 1ストレージプール9ストレージノード3サイト"]
...
image:ec_three_sites_4_plus_2_site_loss_example_template.png["4+2 ECスキームを使用して3つのサイトにオブジェクトをイレイジャーコーディングして格納"]

一方のサイトが失われても、データは引き続きリカバリできます。

image:ec_three_sites_4_plus_2_site_loss_example.png["イレイジャーコーディング4+2 ECスキーム1サイトが失われました"]
