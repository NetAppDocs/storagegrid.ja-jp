...
summary: ネットワークタイムプロトコル（NTP）サーバを追加、更新、または削除して、StorageGRID システムのグリッドノード間でデータが正確に同期されるようにすることができます。 
---
= StorageGRIDでNTPサーバーを管理する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: 運用レベルの StorageGRID インストールに外部 NTP ソースを指定する場合、Windows Server 2016 より前のバージョンの Windows では Windows 時刻 （W32Time） サービスを使用しないでください。以前のバージョンの Windows のタイム サービスは十分な精度を備えておらず、StorageGRID を含む高精度環境での使用は Microsoft によってサポートされていません。詳細については、 https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["高精度時間のサポート境界"^]を参照してください。
