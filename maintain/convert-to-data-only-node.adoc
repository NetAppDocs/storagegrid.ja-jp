...
* ストレージノードにADCサービスが含まれており、グリッド全体で2つ以上のADCストレージノードがオンラインであり、かつ同じサイト上にADCサービスをホストできる非ADCストレージノードが存在する場合。
...
.タスクの内容
この手順を使用して、1つまたは複数の複合ストレージノードをデータ専用ノードに変換します。ストレージノードにADCサービスが含まれている場合、変換中にそのサービスは自動的に同じサイト内の別のストレージノードに移動されます。
...
NOTE: StorageGRID は、ADC サービスを、サービスのホスティング要件を満たす最新のストレージノードに移行します。
