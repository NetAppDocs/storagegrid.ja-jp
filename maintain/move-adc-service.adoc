...
.タスクの内容
ADCサービスをストレージノード間で手動で移動するには、「ADCの移動」手順を使用します。例えば、ADCサービスをノード間で再分配したり、ADCサービスをより高性能なノードに移動したりする場合があります。
...
[NOTE]
====
* ストレージノードのlink:../maintain/convert-to-data-only-node.html["ストレージノードをデータ専用ノードに変換する"]または廃止の前に、ADC サービスを手動で移動する必要はありません。これらの手順では、必要に応じて ADC サービスが自動的に移動されます。要件の詳細については、link:../maintain/understanding-adc-service-quorum.html["ADCの定足数とは何ですか？"]を参照してください。
* StorageGRID は、ADC サービスを、サービスのホスティング要件を満たす最新のストレージノードに移行します。
