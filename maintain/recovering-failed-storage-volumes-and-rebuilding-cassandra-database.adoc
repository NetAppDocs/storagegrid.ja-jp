...
---
= 障害が発生したストレージボリュームを復旧し、StorageGRID で Cassandra データベースを再構築します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.次の手順
ストレージボリュームの再フォーマットと再マウントが完了し、必要な Cassandra 操作が完了したら、link:../maintain/restoring-object-data-to-storage-volume-where-system-drive-is-intact.html["Grid Managerを使用してオブジェクトデータをリストアする"]。
