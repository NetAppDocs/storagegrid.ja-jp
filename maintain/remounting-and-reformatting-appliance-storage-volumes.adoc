...
summary: 自動手順による保存済みストレージボリュームの再マウントおよび障害が発生したストレージボリュームの再フォーマットが失敗した場合は、これらの操作を完了するために2つのスクリプトを手動で実行する必要があります。最初のスクリプトは、StorageGRID ストレージボリュームとして適切にフォーマットされたボリュームを再マウントします。2番目のスクリプトは、マウントされていないボリュームを再フォーマットし、必要に応じて Cassandra データベースを再構築して、サービスを開始します。 
---
= StorageGRIDアプライアンスストレージボリュームの再マウントと再フォーマット（手動手順）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
自動手順による保存済みストレージボリュームの再マウントおよび障害が発生したストレージボリュームの再フォーマットが失敗した場合は、これらの操作を完了するために2つのスクリプトを手動で実行する必要があります。最初のスクリプトは、StorageGRID ストレージボリュームとして適切にフォーマットされたボリュームを再マウントします。2番目のスクリプトは、マウントされていないボリュームを再フォーマットし、必要に応じて Cassandra データベースを再構築して、サービスを開始します。

ストレージノードの復旧中に、Grid Managerに「Waiting for manual steps」というメッセージが表示された場合は、次の手順に従ってストレージボリュームを再マウントおよび再フォーマットしてください。このメッセージが表示されない場合は、再マウントおよび再フォーマット操作は自動的に完了しているため、この手順をスキップしてlink:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["オブジェクトデータを復元する"]に進むことができます。
...
+
image:recovering_cassandra.png["グリッド管理インターフェイスにおけるリカバリの進行状況を示すスクリーンショット"]

. スクリプトでノードのサービスが開始されたら、 `sn-recovery-postinstall.sh`スクリプトでフォーマットされたストレージボリュームにオブジェクトデータをリストアできます。
+
スクリプトは、オブジェクトデータを復元するためにGrid Managerボリューム復元プロセスを使用するかどうかを確認します。

+
** ほとんどの場合、link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["Grid Managerを使用してオブジェクトデータをリストアする"]。回答 `y` Grid Manager を使用します。
** まれなケースとして、交換用ノードのオブジェクトストレージに使用できるボリュームが元のノードよりも少ないことがわかっている場合は、テクニカルサポートに連絡してオブジェクトデータを手動で復元してください。これらのケースのいずれかに該当する場合は、 `n`と回答し、任意のキーを押してコマンドラインに戻ってください。
