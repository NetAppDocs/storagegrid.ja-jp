...
---
= StorageGRIDストレージボリュームの再マウントと再フォーマット（手動手順）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ストレージノードの復旧中に、Grid Manager に「Waiting for manual steps」というメッセージが表示された場合は、次の手順に従ってストレージボリュームを再マウントおよび再フォーマットしてください。このメッセージが表示されない場合は、再マウントおよび再フォーマット操作は自動的に完了しているため、この手順をスキップしてlink:../maintain/restoring-object-data-to-storage-volume.html["オブジェクトデータを復元する"]に進むことができます。
...
+
** ほとんどの場合、link:../maintain/restoring-object-data-to-storage-volume.html["Grid Managerを使用してオブジェクトデータをリストアする"]。回答 `y` Grid Manager を使用します。
