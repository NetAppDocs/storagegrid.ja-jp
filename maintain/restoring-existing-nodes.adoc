...
プライマリ以外の管理ノードからノードを復旧する場合は、復旧したノードがプライマリ管理ノードではなく、プライマリ以外の管理ノードに登録されるように、ADMIN_IP をプライマリ以外の管理ノードのグリッドネットワーク IP アドレスに設定してください。管理ネットワークキーのルールを確認するには、link:../swnodes/creating-node-configuration-files.html["Linux デプロイメント用の StorageGRID ノード構成ファイルを作成する"]を参照してください。
