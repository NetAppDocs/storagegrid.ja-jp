...
summary: '[Decommission Site]ウィザードの手順4（Remove ILM References）では、ILMポリシーで使用されていないルールも含め、サイトを参照している未使用のILMルールを削除または編集する必要があります。' 
---
= StorageGRID のサイトへの ILM 参照を削除します
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
** ルールを削除するには、ゴミ箱アイコン image:icon_trash_can.png["アイコンゴミ箱"] を選択し、*OK* を選択します。
+
