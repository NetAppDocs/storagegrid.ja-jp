...
summary: 運用停止後に残るAdministrative Domain Controller（ADC；管理ドメインコントローラ）サービスが少なすぎる場合は、サイトの一部のストレージノードの運用を停止できないことがあります。 
---
...
ストレージノードを廃止する必要がある場合は、link:../release-notes/changes-to-grid-management-api.html#new-private-endpoints-for-move-adc["ADC サービスを同じサイト内の別のストレージノードに移動する"]またはlink:../expand/index.html["拡張"]で新しいストレージノードを追加してADCサービスを持つように指定することで、ADCクォーラム要件を満たし続けることができます。その後、クォーラムに影響を与えることなく、既存のストレージノードを廃止できます。
