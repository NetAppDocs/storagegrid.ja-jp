...
summary: AutoSupport 機能を使用すると、StorageGRID システムは定期的およびイベント駆動型の健全性とステータスメッセージをテクニカルサポートに送信し、プロアクティブな監視とトラブルシューティングを可能にします。StorageGRID AutoSupport では、予測に基づく推奨機能のために Digital Advisor を使用することもできます。 
---
= StorageGRIDでAutoSupportを設定する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AutoSupport 機能を使用すると、StorageGRID システムは定期的およびイベント駆動型の健全性とステータスメッセージをテクニカルサポートに送信し、プロアクティブな監視とトラブルシューティングを可能にします。StorageGRID AutoSupport では、予測に基づく推奨機能のために Digital Advisor を使用することもできます。

デフォルトでは、ソフトウェアベースの StorageGRID システムに StorageGRID を初めてインストールする際に、週次およびイベントトリガーの AutoSupport 機能が有効になっています。ただし、各アプライアンスで<<autosupport-for-appliances,ハードウェアAutoSupportの設定>>必要があります。必要に応じて、AutoSupport の設定を変更できます。


NOTE: StorageGRID AutoSupport の設定を変更する場合は、プライマリ管理ノードでのみ変更を行ってください。

.開始する前に
* プライマリ管理ノードのGrid Managerにlink:../admin/web-browser-requirements.html["サポートされている Web ブラウザ"]を使用してサインインしています。
* あなたはを持っていますlink:../admin/admin-group-permissions.html["rootアクセス権限"]。
* HTTPSを使用してAutoSupportパッケージを送信する場合は、直接またはlink:../admin/configuring-admin-proxy-settings.html["プロキシサーバを使用する"]のいずれかの方法で、プライマリ Admin ノードへのアウトバウンドインターネットアクセスを提供しています（インバウンド接続は不要です）。
* StorageGRID の AutoSupport ページで HTTP を選択した場合、AutoSupport パッケージを HTTPS として転送するには link:../admin/configuring-admin-proxy-settings.html["プロキシサーバを設定しました"] が必要です。NetApp の AutoSupport サーバは、HTTP を使用して送信されたパッケージを拒否します。
* SMTPをAutoSupportパッケージのプロトコルとして使用する場合は、link:../monitor/configure-email-server.html["メールサーバーを設定しました"]。
...
== [[configure-autosupport]]AutoSupport の設定AutoSupport

AutoSupport のインストールのデフォルト設定を変更するか、各 StorageGRID アプライアンスでハードウェア AutoSupport を設定するには：
...
** *HTTP*：HTTPを選択する場合は、AutoSupportパッケージをHTTPSとして転送するためにlink:../admin/configuring-admin-proxy-settings.html["プロキシ サーバを設定する"]する必要があります。NetAppのAutoSupportサーバは、HTTPを使用して送信されたパッケージを拒否します。このプロトコルはポート80を使用します。
** *SMTP*: このオプションは、AutoSupport パッケージをメールで送信する場合に使用します。まず最初に link:../monitor/configure-email-server.html["メールサーバーの設定"] 必要があります。


+
設定したプロトコルは、HTTPSプロトコルが必要な AutoSupport on Demand を除くすべての AutoSupport パッケージの送信に使用されます。
...
. *SMTP* を選択した場合は、*送信者* フィールドに、以前設定したメールサーバーのアドレスを入力してください。
...
AutoSupportパッケージの送信に使用されるプロトコルを確認または変更するには、<<configure-autosupport,AutoSupportを設定する>>の手順を参照してください。
...
link:sending-eseries-autosupport-messages-through-storagegrid.html["SANtricityシステムマネージャ"]でプロキシ経由のAutoSupport配信を設定すると、Eシリーズ アプライアンスからのAutoSupportパッケージをStorageGRID AutoSupportに含めることができます。
...
.関連情報
* link:../network/external-communications.html["外部との通信に使用するポート"]
