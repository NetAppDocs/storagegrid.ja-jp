...
* stream-access.log：ストリームブロックの TCP/TLS ストリーム（レイヤー 4）プロキシセッション。
+
*注*：これはHTTPリクエストログではありません。
...
| revision_history.json  a| 
改訂履歴。



| routed-s3-access.log.gz  a| 
S3（グローバルネームスペース/フェデレーション URL）のようにルーティングされるグリッドフェデレーショントラフィックのアクセスログ。
...
| system-bucket.log  a| 
`update-system-bucket-configuration.log`：システムバケットのログ。
...
| update-cipher-configurations.log  a| 
`update-cors-configurations.log`：TLS/暗号化方式およびCORSの変更に関連するログをトレースします。



| update-leakd-configurations.log  a| 
孤立検出設定ログ。
