...
+
image:metrics_page.png["メトリクスページ"]

. StorageGRID メトリクスの現在の値を照会したり、時間の経過に伴う値のグラフを表示したりするには、Prometheus セクションのリンクを選択してください。
+
...
. 時間の経過に伴う StorageGRID メトリックのグラフを含む事前構築済みダッシュボードにアクセスするには、Grafana セクションのリンクを選択してください。
+
選択したリンクに対応した Grafana インターフェイスが表示されます。

+
image:metrics_page_grafana.png["指標ページ Grafana"]
