...
* image:icon_alert_green_checkmark.png["緑のチェックマークアイコン"] *正常*：すべての値が正常範囲内です。
* image:icon_alert_yellow_minor.png["アイコンアラート黄マイナー"] *注意*：1つ以上の値が正常範囲外です。
* image:icon_alert_red_critical.png["アイコンアラート赤アラーム"] *注意*：1つ以上の値が正常範囲を大きく逸脱しています。
...
+
image:support_diagnostics_page.png["Support Diagnostics ページ"]

. 特定の診断結果について詳しく知りたい場合は、該当する行を選択してください。
+
...
+
image:support_diagnostics_cpu_utilization.png["診断ステータス値の例"]
...
+
image:grafana_dashboard_nodes.png["Grafana ダッシュボードノード"]

. *オプション*：Prometheus の式の経時的なグラフを表示するには、*Prometheus で表示*を選択します。
+
診断に使用された式の Prometheus グラフが表示されます。

+
image:support_diagnostics_prometheus_png.png["Prometheus ページの診断をサポートします"]
