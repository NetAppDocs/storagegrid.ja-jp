...
image:grid_manager_dashboard_and_menu.png["Grid Manager のダッシュボード"]

各カードに表示されている情報の説明については、そのカードのヘルプアイコンimage:icon_nms_question.png["疑問符アイコン"]を選択してください。
...
ヘルプメニューimage:icon_help_menu_bar.png["メニューバーのヘルプアイコン"]からアクセスできる項目：
...
* link:../admin/grid-federation-overview.html["グリッドフェデレーション"]を使用して、テナントアカウント情報を同期し、2つのStorageGRIDシステム間でオブジェクトデータをレプリケートします
...
* link:../monitor/configure-email-server.html["メールサーバーの設定"]
...
* link:../admin/about-multi-admin-verify.html["複数管理者による検証を設定する"]
...
* link:../monitor/configure-autosupport-grid-manager.html["AutoSupportの設定"]
