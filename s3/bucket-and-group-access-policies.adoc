...
ifExists
 a| 
Null条件以外の条件演算子に追加され、その条件キーがないかどうかを確認します。Conditionキーが存在しない場合はtrueを返します。
...
S3 ： PutObject
 a| 
は、PutObject、CopyObject、およびCreateMultipartUpload要求の要求ヘッダーから展開されたと比較し `object-lock-mode`ます。
...
S3 ： PutObjectRetention
 a| 
は、PutObjectRetention要求のXML本文から展開されたと比較し `object-lock-mode`ます。
...
S3 ： PutObject
 a| 
は、PutObject、CopyObject、CreateMultipartUpload、UploadPart、UploadPartCopy、およびCompleteMultipartUpload要求の要求ヘッダーから展開されたまたはと `copy-source-sse-customer-algorithm`比較します。 `sse-customer-algorithm`
