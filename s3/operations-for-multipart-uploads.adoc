...
* マルチパートアップロードされたオブジェクトがlink:../tenant/what-is-branch-bucket.html["ブランチバケット"]に含まれるかどうかは、取り込みが開始された時刻と、ブランチバケットのタイムフィルターによって異なります。
+
** *前に*。インジェストがブランチバケットの_Before_タイムスタンプより前に開始された場合、アップロードが完了した時期に関係なく、マルチパートアップロードオブジェクトが含まれます。
** *後*。ブランチバケットの_After_タイムスタンプ以降に取り込みが開始された場合、マルチパートアップロードされたオブジェクトが含まれます。
** *間*。ブランチバケットの_From_タイムスタンプと_To_タイムスタンプの間に取り込みが開始された場合、アップロードが完了したタイミングに関係なく、マルチパートアップロードオブジェクトが含まれます。
** *除外する*。取り込みがブランチバケットの「除外開始」時刻より前、または「除外終了」時刻より後に開始された場合、マルチパートアップロードオブジェクトが含まれます。
