...
*  `If-Match`および `If-None-Match`ヘッダーの動作は、StorageGRIDの構成によって異なります。これらのヘッダーを指定すると、エラーメッセージ（501 Not Implemented）が返される場合があります。 `If-Match`または `If-None-Match`ヘッダーが受け入れられても機能しない場合、より厳格な501 Not Implementedエラー動作を有効にする手順を含むアラートがトリガーされる可能性があります。
