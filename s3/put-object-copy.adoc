...
* `Content-Type`
* `x-amz-checksum-algorithm`： `CRC32`、 `CRC32C`、 `CRC64NVME`、または `SHA256`を指定します
+
オブジェクトをコピーする場合、ソースオブジェクトに `checksum`が含まれているか、または `x-amz-checksum-algorithm`が含まれている場合、コピーされたオブジェクトにはFULL_OBJECTチェックサムが付与されます。

+
ソースオブジェクトにチェックサムがあるが、リクエストに `x-amz-checksum-algorithm`が含まれていない場合、またはサポートされていないチェックサム値を使用している場合、コピーされたオブジェクトはソースオブジェクトと同じタイプのチェックサムを持ちます。StorageGRID は、無効またはサポートされていない `x-amz-checksum-algorithm`値を無視します。

+
ソースオブジェクトにチェックサムがなく、 `x-amz-checksum-algorithm`が含まれていない場合、またはサポートされていないチェックサム値を使用している場合、コピーされたオブジェクトにはチェックサムがありません。

+

NOTE: CopyObjectリクエストでチェックサムが指定されておらず、ソースオブジェクトにもチェックサムがない場合、StorageGRIDはオブジェクトのデフォルトのCRC-64/NVMEチェックサムを計算しません。
