...
このページには、StorageGRID でサポートされている S3 操作のみが記載されています。リストに記載されていない S3 操作は、 `501 Not Implemented`が返されるか、意図しない動作が発生する可能性があります。


TIP: 各処理のAWSドキュメントを参照するには、見出しのリンクを選択します。
...
.URIクエリパラメータと要求ヘッダー
StorageGRID は、このリクエストに対して_ほとんどの_<<common-params,共通のパラメータとヘッダー>>をサポートしています。

.リクエストの本文
StorageGRID は、実装時点で Amazon S3 REST API によって定義されたリクエストボディパラメータの_ほとんど_をサポートしていますが、以下の例外があります：

グリッド管理者から許可が与えられていない限り、PutObjectLockConfiguration を使用して既存のバケットで S3 オブジェクトロックを有効にすることはできません。
