...
summary: 'StorageGRID は Amazon Web Services (AWS) のポリシー言語を使用して、S3 テナントがバケットおよびバケット内のオブジェクトへのアクセスを制御できるようにします。' 
---
...
* オブジェクトキーで指定できる国際文字は、JSON UTF-8 または JSON \u エスケープシーケンスを使用してエンコードする必要があります。パーセントエンコーディングはサポートされていません。
+
...
[[use-putoverwriteobject-permissionuse-putoverwriteobject-permission]]
== [[use-putoverwriteobject-permission]]PutOverwriteObject 権限を使用する
...
存在する場合
 a| 
Null条件を除く、任意の条件演算子に追加して、その条件キーが存在しないかどうかを確認します。条件キーが存在しない場合はTRUEを返します。
...
IP 演算子
 a| 
リクエストが送信されたIPアドレスと比較します。バケット操作またはオブジェクト操作に使用できます。

*注:* S3 リクエストが管理ノードおよびゲートウェイノードのロードバランサーサービスを経由して送信された場合、これはロードバランサーサービスの上流の IP アドレスと比較されます。

*注*: サードパーティ製の非透過型ロードバランサーを使用する場合は、そのロードバランサーのIPアドレスと比較します。 `X-Forwarded-For`ヘッダーは有効性を確認できないため、無視されます。
...
S3 ： PutObjectVersionTagging
 a| 
既存のオブジェクトに、特定のタグキーと値が存在する必要があります。
...
S3 ： ListBucketVersions 権限
 a| 
ListObjectsまたはListObjectVersionsリクエストで指定されたプレフィックスパラメータ、あるいはGetObjectまたはHeadObjectに指定されたキーと比較します。
...
S3 ： PutObjectVersionTagging
 a| 
オブジェクト要求にタグ付けが含まれる場合、特定のタグキーと値が必要です。
...
S3 ： PutObject
 a| 
リクエストヘッダーから展開された x-amz-copy-source を PutObject および UploadPartCopy リクエストと比較します。
...
NOTE: DeleteObject を S3 ポリシーで DENY に設定しても、「30日後にコピーをゼロにする」などのルールが存在する場合、ILM がオブジェクトを削除するのを防ぐことはできません。
