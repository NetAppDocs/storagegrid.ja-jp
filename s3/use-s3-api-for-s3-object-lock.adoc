...
NOTE: StorageGRIDは `x-amz-sdk-checksum-algorithm`リクエストヘッダーをサポートしていません。
...
バケットに対してS3オブジェクトロックを有効にするには、次のいずれかの方法を使用します。
...
* テナントマネージャーからバケット設定を管理します。link:../tenant/update-default-retention-settings.html["S3オブジェクトロックのデフォルトの保持期間を更新します"]を参照してください。
...
新しいオブジェクトバージョンがバケットに取り込まれると、 `x-amz-object-lock-mode`と `x-amz-object-lock-retain-until-date`が指定されていない場合、デフォルトの保持モードが適用されます。 `x-amz-object-lock-retain-until-date`が指定されていない場合、デフォルトの保持期間を使用して保持期限日が計算されます。
...
PutObjectLockConfiguration リクエストには、以下のリクエストヘッダーのいずれかを指定する必要があります：
...
NOTE: StorageGRID は、この操作では x-amz-checksum-* ヘッダーの内容を検証しません。



==== 要求例

この例では、バケットに対してS3オブジェクトロックを有効にし（グリッド管理者が許可を与えている場合）、デフォルトの保持モードをCOMPLIANCEに、デフォルトの保持期間を6年に設定します。
...
* PutObject リクエストに x-amz-object-lock-* リクエストヘッダーが含まれている場合は、次のリクエストヘッダーのいずれかを含める必要があります：
+
[listing]
----
Content-MD5
...
----
+
これらのヘッダーは、CopyObject または CreateMultipartUpload には必要ありません。

* バケットで S3 オブジェクトロックが有効になっておらず、x-amz-object-lock-* リクエストヘッダーが存在する場合、400 Bad Request （InvalidRequest）エラーが返されます。
...
リクエストには、以下のいずれかのリクエストヘッダーを指定する必要があります。
...
NOTE: StorageGRID は、これらの操作では x-amz-checksum-* ヘッダーの内容を検証しません。
...
+
 `x-amz-bypass-governance-retention`ヘッダーは、S3 Object Lockのないオブジェクトには影響を与えません。
