...
* レプリケーションは、Amazon S3 または StorageGRID S3 バケットで S3 オブジェクトロックが有効になっているソースバケットまたは宛先バケットに対してのみサポートされます。オブジェクトロックヘッダーは、宛先オブジェクトにレプリケートされません。
...
* CloudMirror レプリケーションを設定した後に宛先バケットで S3 オブジェクトロックを有効にした場合、レプリケーションを再開できるようにするために link:../tenant/editing-platform-services-endpoint.html["エンドポイントを再保存する"] が必要になることがあります。
