...
単一のS3バケットに対して、プラットフォームサービスの任意の組み合わせを設定できます。たとえば、StorageGRID S3バケットでlink:../tenant/understanding-cloudmirror-replication-service.html["CloudMirrorサービス"]とlink:../tenant/understanding-notifications-for-buckets.html["通知"]の両方を設定することで、特定のオブジェクトをAmazon Simple Storage Service （S3）にミラーリングしながら、AWSの費用を追跡するためのサードパーティの監視アプリケーションに各オブジェクトの通知を送信できます。
...
バージョン管理を使用する場合、S3プロトコルの制限により、宛先バケット内のオブジェクトバージョンの順序はベストエフォートであり、CloudMirror サービスによって保証されるものではないことに注意してください。また、宛先バケットは独自の設定を保持するため、バージョン管理機能を提供しない場合があります。
...
S3 オブジェクトのロックが有効になっているバケット
 a| 
StorageGRID のソース S3 バケットで S3 Object Lock が有効になっている場合は、デフォルトの保持期間が設定された宛先バケットに対しても S3 Object Lock を有効にする必要があります。

S3 Object Lockが有効になっているソースバケットと宛先バケットに対して、_オブジェクトデータ_のレプリケーションがサポートされます。ただし、宛先バケットはオブジェクト保持に関する独自の設定を維持するため、コンプライアンスやバージョン管理を提供しない場合があります。
