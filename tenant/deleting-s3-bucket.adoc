...
S3バケットにオブジェクト、最新ではないオブジェクトバージョン、または削除マーカーが含まれている場合、そのバケットを削除することはできません。S3バージョン管理されたオブジェクトの削除方法については、link:../ilm/how-objects-and-delete-markers-are-deleted.html["オブジェクトと削除マーカーの削除方法"]を参照してください。
