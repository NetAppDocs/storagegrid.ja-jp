...
image:grid-federation-account-clone-workflow-tm.png["アカウントクローンのテナントのワークフロー。手順については、次のテキストで説明します。"]
...
image:grid-federation-account-clone.png["ローカルグループがソースグリッドからデスティネーショングリッドにクローニングされることを示すイメージ"]
...
image:grid-federation-local-user-clone.png["ローカルユーザがソースグリッドからデスティネーショングリッドにクローンされていることを示すイメージ"]
...
image:grid-federation-federated-group-clone.png["フェデレーテッドグループがソースグリッドからデスティネーショングリッドにクローニングされたことを示す図"]
...
image:grid-federation-s3-access-key.png["s3アクセスキーをソースグリッドからデスティネーショングリッドにオプションでクローニングできることを示すイメージ"]
...
[[groups-and-users-added-to-destination-grid-arent-cloned]]
...
image:grid-federation-account-not-cloned.png["デスティネーショングリッドの詳細がソースグリッドにクローンされていないことを示す画像"]



[[edited-or-deleted-groups-users-and-access-keys-arent-cloned]]
...
image:grid-federation-account-clone-edit-delete.png["編集または削除された詳細が複製されていないことを示す画像"]
