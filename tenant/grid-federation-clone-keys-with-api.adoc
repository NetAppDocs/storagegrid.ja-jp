...
summary: StorageGRIDテナントアカウントにUse grid federation connection権限がある場合は、Tenant Management APIを使用してS3アクセスキーを手動でクローニングできます。 
---
...
+
image:grid_federation_post_current_user_replicate.png["自身のアクセスキーをレプリケートするためのPOSTエンドポイント"]
...
+
image:grid_federation_clone_access_key.png["テナントマネージャAPI：アクセスキーをクローニングするための値を入力しています"]
...
[[clone-another-users-access-keys]]
...
+
image:grid_federation_post_other_user.png["別のユーザのアクセスキーをレプリケートするためのエンドポイントをPOSTします"]
