...
+
image:grid-federation-post-current-user-replicate.png["自身のアクセスキーをレプリケートするためのPOSTエンドポイント"]
...
+
image:grid-federation-clone-access-key.png["テナントマネージャAPI：アクセスキーをクローニングするための値を入力しています"]
...
+
image:grid-federation-post-other-user.png["別のユーザのアクセスキーをレプリケートするためのエンドポイントをPOSTします"]
