...
image:tenant_dashboard_endpoint_error.png["ダッシュボードのエンドポイントエラーメッセージ"]
...
. エンドポイントの詳細ページで、「接続」を選択します。このタブには、エンドポイントに関する最新のエラーのみが表示され、エラーが発生した日時も示されます。赤いXアイコンimage:icon_alert_red_critical.png["赤い X 印のアイコン"]を含むエラーは、過去7日以内に発生したものです。
