...
---
= StorageGRIDの管理ノードとユーザーインターフェースの問題のトラブルシューティング
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. StorageGRID システムでシングル サインオン（SSO）が有効になっている場合は、link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"]またはlink:../admin/learn-about-setting-up-sso-saml.html["SAML"]を使用した SSO の設定を参照してください。
+
...
... 「接続テスト」を選択して、LDAPサーバの接続設定を検証します。
