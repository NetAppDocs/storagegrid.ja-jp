...
summary: 低オブジェクト データ ストレージ アラートは、各ストレージ ノードでオブジェクト データを格納するために使用できるスペースの量を監視します。 
---
= StorageGRID でオブジェクトデータストレージ不足の警告を解決する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:nodes_page_storage_used_object_data.png["Nodes ページ - ストレージの使用済みオブジェクトデータ"]
