...
[[troubleshootmtualertgrid-network-mtu-mismatch-alert]]
...
NOTE: Linuxホスト OS で使用する仮想ネットワークインターフェースを個別に作成してください。Linux ホスト OS と StorageGRID コンテナに同じネットワークインターフェースを使用すると、ホスト OS にアクセスできなくなる可能性があります。
